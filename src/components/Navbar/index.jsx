...
export function Navbar({ titulo, user, onSelectImovel }) {
  ...
  const displayName = localStorage.getItem("userName") ?? user?.nome ?? "Usuário";

  return (
    <>
      {isNavMenuOpen && <NavMenu closeNav={handleNavStateClick} />}

      <header className="fixed top-0 left-0 z-50 w-full px-[18px] pt-[30px] pb-[14px] flex flex-col gap-3 text-white bg-gradient-to-b from-[#7B61FF] to-[#A391FF] font-sans">
        ...
        <div className="relative w-full z-50">
            <div className="w-full bg-white/20 rounded-[20px] px-[12px] py-[9px] flex items-center gap-[10px]">
              <img src="/search.png" alt="search" className="opacity-90 w-8! h-6!" />
              ...
              {loading ? (
                 ...
                 <img src="/mic.png" alt="mic" className="w-6! h-6! opacity-90" />
              )}
            </div>

            {showDropdown && results.length > 0 && (
              <div className="absolute top-full left-0 w-full mt-1 bg-gradient-to-b from-[#7B61FF] to-[#A391FF] rounded-b-[20px] rounded-t-[5px] shadow-xl">
                <ul className="max-h-[300px] overflow-y-auto scrollbar-hide">
                  {results.map((imovel) => (
                    <li 
                      key={imovel.id}
                      className="flex items-center gap-3 px-4 py-3 border-b border-white/10 hover:bg-white/20 cursor-pointer"
            ...
            {showDropdown && query.length > 0 && results.length === 0 && !loading && (
                 <div className="absolute top-full left-0 w-full mt-1 bg-gradient-to-b from-[#7B61FF] to-[#A391FF] rounded-[15px] p-4 text-white/90 text-center text-sm shadow-xl">
